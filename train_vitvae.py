...
def parse_args():
    parser = argparse.ArgumentParser(
        description="Simple example of a ViT-VAE training script."
    )
    parser.add_argument(
    ...
    parser.add_argument(
        "--output_dir",
        type=str,
        default="vit-vae-model-finetuned",
    ...
    parser.add_argument(
        ...
        type=int,
        default=8,
        help="Downsample factor of VAE",
    )
    parser.add_argument(
        "--project_channels",
        type=int,
        default=4,
        help="Number of VAE channels for alignment with ViT",
    )
...
def main():
    ...
    vit = AutoModel.from_pretrained(
        args.pretrained_vision_model_name_or_path, revision=args.revision
    ).vision_model
    vit.requires_grad_(False)
    
    # Set projector
    latent_size = args.resolution // args.downsample
    patch_size = latent_size // (vit.config.image_size // vit.config.patch_size)
    projector = torch.nn.Conv2d(
        in_channels=args.project_channels,
        out_channels=vit.config.hidden_size,
    ...
    (
        ...
        vae, projector, optimizer, train_dataloader, test_dataloader, lr_scheduler
    )
    ...
    vit.to(accelerator.device)
    projector.to(accelerator.device, dtype=weight_dtype)
    ...
    for epoch in range(first_epoch, args.num_train_epochs):
        ...
        for step, batch in enumerate(train_dataloader):
            with accelerator.accumulate(vae):
                ...
                project_z = projector(z[:, :args.project_channels]).flatten(2).transpose(1, 2)
                with torch.no_grad():
    ...
    if accelerator.is_main_process:
        ...
        projector = accelerator.unwrap_model(projector)
        torch.save(projector, os.path.join(args.output_dir, "projector.pth"))
